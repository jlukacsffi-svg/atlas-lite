...
- Next: verify that the next scheduled daily run establishes the prospective
  study marker, replaces the pre-fix all-zero movement snapshot, and
  automatically adds the first report generated by the aligned runtime. Then
  use the new forward evidence to improve defensive review measurement without
  changing paper authority.
